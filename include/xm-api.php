...
function add_get_blog_info()
{
    ...
    $newComment = get_comments(array(
   	...
   	$latestComment = array();
  	/*var_dump($newComment);
  	var_dump("https://www.gravatar.com/avatar/" . md5(strtolower(trim($newComment[0]->comment_author_email))) . "?s=200");
  	exit;
	...
	for ($i = 0; $i < count($newComment); $i++) {
      	preg_match("/\d/", md5($newComment[$i]->comment_author_email), $matches);
        $tmp = [
            'avatar'=> "https://www.gravatar.com/avatar/" . md5(strtolower(trim($newComment[$i]->comment_author_email))) . "?s=200",
            'background' => $colors[$matches[0]], // 根据邮箱md5后获取第一个数字生成颜色
            'countCom' => get_comments_number($newComment[$i]->comment_post_ID),
            'link' => get_post_meta($newComment[$i]->comment_post_ID, "xm_post_link", true)["very_good"],
...
function add_api_get_phrase ()
{
    ...
    for ($i = 0; $i < count($list); $i++) {
        ...
        $result[$i]->avatar = get_option("xm_vue_options")["domain"] . replace_domain(get_the_author_meta("simple_local_avatar", $list[$i]->post_author)[full]);
    }
    return array(
        "success" => true,
        "data" => $result
...
function add_api_get_links ($request)
{
    ...
    /*for ($i = 0; $i < count($list); $i++) {
  	...
  	for ($i = 0; $i < count($list); $i++) {
       $tmp = [
        'text' => $list[$i]->link_name,
        'url' => $list[$i]->link_url,
        'target' => $list[$i]->link_target,
        'image' => $list[$i]->link_image,
        'description' => $list[$i]->link_description,
        'notes' => $list[$i]->link_notes,
        'rss' => $list[$i]->link_rss,
      ];
 		$result[] = $tmp;
	}
...
function add_api_comment_meta_field()
{
    ...
    register_rest_field("comment", "userAgentInfo", array(
        "get_callback" => function ($object, $field_name, $request) {
          	...
          	if (!get_metadata("comment", $object[id], "userAgentInfo", true)) {
                preg_match("/\d/", md5($object[author_email]), $matches);
                $array = array(
                    "userAgent" => get_browser_name($value[author_user_agent]),
                    "vipStyle" => get_author_class($value[author_email]),
                    "author_avatar_urls" => "https://www.gravatar.com/avatar/" . md5(strtolower(trim($value[author_email]))) . "?s=200",
                    "background" => $colors[$matches[0]] // 根据邮箱md5后获取第一个数字生成颜色
                );
                add_metadata("comment", $object[id], "userAgentInfo", $array, true);
            }
          	 
          	//var_dump(get_metadata("comment", $object[id], "userAgentInfo", true));
          	//exit;
            return get_metadata("comment", $object[id], "userAgentInfo", true);
        },
        "schema" => null
    ));
